...
import { INITIAL_MARKET_RATES, ALL_INDIAN_STATES, MarketRate } from '../data/seedData.js';
...
router.get('/', (req: Request, res: Response) => {
  const { state, mandi, category, search } = req.query;

  let result = [...ratesStore];
  const selectedState = typeof state === 'string' ? state : 'All';
  const selectedMandi = typeof mandi === 'string' ? mandi : 'All';
